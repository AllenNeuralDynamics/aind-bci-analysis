...
from bci_analysis.io.io_suite2p import sessionwise_to_trialwise


def collapse_dlc_data(
    dlc_data: pd.DataFrame, target_length: int = 0, mode="edge"
):
    ...
    for bp in bodyparts:
        for dim in ["x", "y"]:
            if mode == "zero":
                ...
                list_bp = (
                    list(dlc_data[bp][dim])
                    + [np.mean(dlc_data[bp][dim][-3:])] * pad_width
                )
            df2[bp, dim] = (
                np.asarray(list_bp).reshape(-1, mean_window).mean(axis=1)
            )
    return df2
...
def get_aligned_data(
    ...
    Example
    -------
    dlc_base_dir = os.path.abspath("bucket/Data/Behavior_videos/DLC_output/"
                                   "Bergamo-2P-Photostim/")
    bpod_path = os.path.abspath("bucket/Data/Behavior/BCI_exported/"
                                "Bergamo-2P-Photostim/")
    suite2p_path = os.path.abspath("bucket/Data/Calcium_imaging/suite2p/"
                                   "Bergamo-2P-Photostim/")
    sessionwise_data_path = os.path.abspath("bucket/Data/Calcium_imaging/"
                                            "sessionwise_tba/")
    mouse = "BCI_26"
    FOV = "FOV_04"
    camera = "side"
    session = "041022"
    F_aligned, DLC_aligned = get_aligned_data(suite2p_path, dlc_base_dir,
                                    bpod_path, sessionwise_data_path, mouse,
                                    FOV, camera, session)
    """
    os.makedirs(os.path.join(aligned_data_path, mouse), exist_ok=True)
    dict_save_path = os.path.join(
        aligned_data_path, mouse, f"{session}-dict_aligned.npy"
    )
    if os.path.isfile(dict_save_path) and (overwrite is False):
    ...
    with open(
        os.path.join(suite2p_path, mouse, FOV, session, "filelist.json")
    ) as f:
        filelist = json.load(f)

    cl_trial_list = [
        filelist["file_name_list"][i]
        for i in range(len(filelist["frame_num_list"]))
        if filelist["file_name_list"][i].startswith("neuron")
    ]
    print(
        len(cl_trial_list), len(behavior_movie_names), len(trial_start_times)
    )

    F_trialwise = sessionwise_to_trialwise(
        F,
        ca_data["all_si_filenames"],
        ca_data["closed_loop_filenames"],
        ca_data["all_si_frame_nums"],
        ca_data["sampling_rate"],
        align_on="trial_start",
        max_frames="all",
    )
    ...
    for i, bm_name in tqdm(enumerate(behavior_movie_names)):
        ...
        if len(camera_movies) == 0:
            ...
            print(
                f"Multiple {camera} camera files found for trial {i}, skipping"
            )
            continue

        video_path = camera_movies[0]
        dlc_file_name = video_path[
            video_path.find(camera) + len(camera) + 1 :
        ].split(
            "/"
        )  # [mouse, session_id, trial_id]
        dlc_folder = os.path.join(
            dlc_base_dir, camera, dlc_file_name[0], dlc_file_name[1]
        )
        ...
        trial_csv = [
            k
            for k in next(os.walk(dlc_folder))[2]
            if k.startswith(trial_id) and k.endswith("csv")
        ][0]
        trial_json = [
            k
            for k in next(os.walk(dlc_folder))[2]
            if k.startswith(trial_id) and k.endswith("json")
        ][0]

        frame_times_dlc = np.asarray(trial_metadata["frame_times"])
        frame_times_ca = np.arange(0, F_trial.shape[1], 1, dtype=float) / fs

        dlc_trial = pd.read_csv(
            os.path.join(dlc_folder, trial_csv), header=[1, 2], index_col=0
        )
        ...
        if frame_times_dlc[-1] > frame_times_ca[-1]:
            closest_id = np.argmin(
                np.abs(frame_times_dlc - frame_times_ca[-1])
            )
        ...
        if frame_times_dlc[-1] < frame_times_ca[-1]:
            closest_id = np.argmin(
                np.abs(frame_times_ca - frame_times_dlc[-1])
            )
        ...
        lt.append(
            list((lick_times[i]) * (dlc_trial.shape[0] / trial_times[i]))
        )
