...
def find_pybpod_sessions(subject_names_list, date_now, projects):
            ...
            for stp in stps:
                ...
                for session in stp.sessions:
                    for subject_now in subject_names_list:
                        if (
                            session.subjects
                            and session.subjects[0].find(subject_now) > -1
                            and session.name.startswith(date_now)
                        ):
    ...
    outdict = {
        "sessions": np.asarray(sessions_now)[order],
...
def export_pybpod_files_core(
    bpod_session_dict, calcium_imaging_raw_session_dir, zaber_root_folder
):  #
    """
    ...
    for sessionfile in bpod_session_dict["sessions"]:
        ...
        zaber_dict = io.io_pybpod.add_zaber_info_to_pybpod_dict(
            behavior_dict, subject_name, setup_name, zaber_root_folder
        )

        for key in zaber_dict.keys():
            behavior_dict["zaber_{}".format(key)] = zaber_dict[key]
        trialnum = len(behavior_dict["trial_num"])
        behavior_dict["bpod_file_names"] = np.asarray(
            [sessionfile.filepath.split("/")[-1]] * trialnum
        )

        behavior_dict["subject_name"] = np.asarray([subject_name] * trialnum)
        behavior_dict["setup_name"] = np.asarray([setup_name] * trialnum)
        behavior_dict["experimenter_name"] = np.asarray(
            [experimenter_name] * trialnum
        )
        if trialnum > 0:
            behavior_dict_list.append(behavior_dict)
            sessionfile_start_times.append(
                behavior_dict["trial_start_times"][0]
            )
    ...
    for basename in uniquebasenames:
        file_idxs_now = (files["exts"] == ".tif") & (
            files["basenames"] == basename
        )
        ...
        for filename in filenames:
            try:
                metadata = io.io_scanimage.extract_scanimage_metadata(
                    os.path.join(calcium_imaging_raw_session_dir, filename)
                )
            # TODO: Don't use bare except.
            except:  # noqa: E722
                print(
                    "tiff file read error: {}".format(
                        os.path.join(calcium_imaging_raw_session_dir, filename)
                    )
                )
                continue
            ...
            if (
                float(
                    metadata["description_first_frame"]["frameTimestamps_sec"]
                )
                < 0
            ):
                valence = -1
                ...
                valence = 1
            frame_timestamp = movie_start_time + valence * datetime.timedelta(
                seconds=np.abs(
                    float(
                        metadata["description_first_frame"][
                            "frameTimestamps_sec"
                        ]
                    )
                )
            ...
            try:
                ...
                integration_roi_data["outputChannelsRoiNames"] = eval(
                    metadata["metadata"]["hIntegrationRoiManager"][
                ...
                integration_roi_data["outputChannelsFunctions"] = eval(
                    metadata["metadata"]["hIntegrationRoiManager"][
    ...
    scanimage_integration_roi_data_all = np.asarray(
        scanimage_integration_roi_data_all
    )
    ...
    scanimage_integration_roi_data_all = scanimage_integration_roi_data_all[
        file_order
    ]
    scanimage_metadata_dict_list = scanimage_metadata_dict_list[file_order]

    istriggered = list()
    for stamp in trigger_arrived_timestamps_all:
        istriggered.append(type(stamp) == datetime.datetime)
    ...
    if len(dist_list) > 0:
        ...
        dist_list = dist_list[
            (dist_list > center_sec - 1) & (dist_list < center_sec + 1)
        ]
        ...
        for trial_start_time, trial_end_time in zip(
            behavior_dict["trial_start_times"],
            behavior_dict["trial_end_times"],
        ):
            trial_start_time = trial_start_time + datetime.timedelta(
                seconds=time_offset - 0.5
            )  # gets a 0.5 second extra
            trial_end_time = trial_end_time + datetime.timedelta(
                seconds=time_offset
            )
            movie_idxes = (
                trigger_arrived_timestamps_all[istriggered] > trial_start_time
            ) & (trigger_arrived_timestamps_all[istriggered] < trial_end_time)
            if any(movie_idxes):
                if sum(movie_idxes) == 1:
                    moviename = np.asarray(
                        filenames_all[istriggered][movie_idxes]
                    )
                    scanimage_metadata = np.asarray(
                        scanimage_metadata_dict_list[istriggered][movie_idxes]
                    )
                else:
                    moviename = np.asarray(
                        filenames_all[istriggered][movie_idxes]
                ...
                movie_trial_time_offset = (
                    trigger_arrived_timestamps_all[istriggered][movie_idxes][0]
                    - (
                        trial_start_time
                        - datetime.timedelta(seconds=time_offset - 0.5)
                    )
                ).total_seconds()
                trigger_to_frame_offset = (
                    frame_timestamps_all[istriggered][movie_idxes][0]
                    - trigger_arrived_timestamps_all[istriggered][movie_idxes][
                        0
                    ]
                ).total_seconds()
                roidata = scanimage_integration_roi_data_all[istriggered][
                    movie_idxes
                ][0]
            else:
        ...
        behavior_dict["scanimage_bpod_time_offset"] = np.asarray(
            bpod_scanimage_time_offset
        )
        behavior_dict["scanimage_first_frame_offset"] = np.asarray(
            scanimage_frame_time_offset
        )
        behavior_dict["scanimage_tiff_headers"] = np.asarray(
            scanimage_metadata_list
        )
    else:
    ...
    residual_tiff_files = {
        "median_bpod_si_time_offset": time_offset,
        "triggered": list(),
    ...
    skip_metadata = False
    for scanimage_fname, scanimage_timestamp, scanimage_metadata in zip(
        residual_filenames, residual_timestamps, residual_metadata
    ):
        if type(scanimage_timestamp) == float:
            residual_tiff_files["triggered"].append(False)
            movie_start_time_now = frame_timestamps_all[
                filenames_all == scanimage_fname
            ][0] - datetime.timedelta(seconds=time_offset)
        else:
            residual_tiff_files["triggered"].append(True)
            movie_start_time_now = scanimage_timestamp - datetime.timedelta(
                seconds=time_offset
            )
        if any(behavior_dict["trial_start_times"] > movie_start_time_now):
            next_trial_idx = np.where(
                behavior_dict["trial_start_times"] > movie_start_time_now
            )[0][0]
            time_to_next_trial = (
                behavior_dict["trial_start_times"][next_trial_idx]
                - movie_start_time_now
        ...
        if any(behavior_dict["trial_start_times"] < movie_start_time_now):
            prev_trial_idx = np.where(
                behavior_dict["trial_start_times"] < movie_start_time_now
            )[0][-1]
            time_from_prev_trial = (
                movie_start_time_now
                - behavior_dict["trial_start_times"][prev_trial_idx]
            ).total_seconds()  # - time_offset
        else:
            prev_trial_idx = np.nan
            time_from_prev_trial = np.nan

        residual_tiff_files["time_from_previous_trial_start"].append(
            time_from_prev_trial
        )
        residual_tiff_files["time_to_next_trial_start"].append(
            time_to_next_trial
        )
        ...
        if "slm" in scanimage_fname and not skip_metadata:
            residual_tiff_files["scanimage_tiff_headers"].append(
                scanimage_metadata
            )
            skip_metadata = True
...
def export_single_pybpod_session(
    session,
    subject_names,
    save_dir,
    ...
    """
    Main function that exports a single pybpod csv file, pairs them with
    scanimage tiffs and zaber json files and exports them all in a trial-based
    manner.
    ...
    subject_names : list of str
        names of subjects to export, or if the subject has multiple aliases,
        add all
    save_dir : str
    ...
    BCI_analysis.pipeline_bpod.export_single_pybpod_session(session = '040122',
                                 ...
                                 calcium_imaging_raw_session_dir = (
                                   '/mnt/Data/Calcium_imaging/raw/KayvonScope/'
                                   'BCI_26/040122'),
                                 raw_behavior_dirs = (
                                   ['/mnt/Data/Behavior/raw/KayvonScope/BCI']),
                                 zaber_root_folder = (
                                 '/mnt/Data/Behavior/BCI_Zaber_data/'
                                 'KayvonScope'))
    ...
    """
    # TODO: Use regex matching instead of nested tries.
    try:
    ...
    # TODO: Don't use bare except.
        ...
        # TODO: Don't use bare except.
            ...
            # TODO: Don't use bare except.
                ...
                print(
                    "cannot understand date for session dir: {} - "
                    "should be a date".format(session)
                )
            # continue
    ...
    bpod_session_dict = find_pybpod_sessions(
        subject_names, session_date.strftime("%Y%m%d"), projects
    )
    behavior_dict = export_pybpod_files_core(
        bpod_session_dict, calcium_imaging_raw_session_dir, zaber_root_folder
    )
    ...
    behavior_dict_matlab["trial_start_times"] = np.asarray(
        behavior_dict["trial_start_times"], str
    )
    behavior_dict_matlab["trial_end_times"] = np.asarray(
        behavior_dict["trial_end_times"], str
    )
    behavior_dict_matlab.pop("scanimage_tiff_headers")
    try:
        behavior_dict_matlab["residual_tiff_files"].pop(
            "scanimage_tiff_headers"
        )
    # TODO: Don't use bare except.
    except:  # noqa: E722
        pass
...
# this script will export behavior and pair it to imaging, then save it in a
# neat directory structure
def export_pybpod_files(
    behavior_export_basedir,
    ...
    for setup in setups[::-1]:
        calcium_imaging_raw_setup_dir = os.path.join(
            calcium_imaging_raw_basedir, setup
        )
        subjects = np.sort(os.listdir(calcium_imaging_raw_setup_dir))
        for subject in subjects:
            if "bci" in subject.lower():
                calcium_imaging_raw_subject_dir = os.path.join(
                    calcium_imaging_raw_setup_dir, subject
                )
                subject_wr_name = "BCI{}".format(
                    subject[subject.find("_") + 1 :]
                )
                ...
                for session in sessions:
                    ...
                    calcium_imaging_raw_session_dir = os.path.join(
                        calcium_imaging_raw_subject_dir, session
                    )
                    bpod_export_dir = os.path.join(
                        behavior_export_basedir, setup, subject
                    )
                    Path(bpod_export_dir).mkdir(parents=True, exist_ok=True)
                    bpod_export_file = "{}-bpod_zaber.npy".format(session)
                    if not overwrite and os.path.exists(
                        os.path.join(bpod_export_dir, bpod_export_file)
                    ):
                        print(
                            "session already exported: {}".format(
                                os.path.join(bpod_export_dir, bpod_export_file)
                            )
                        )
                        continue
                    ...
                    print(
                        "exporting behavior from {}/{}".format(
                            subject, session
                        )
                    )
                    try:
                    ...
                    # TODO: Don't use bare except.
                        ...
                        print(
                            "COULD NOT EXPORT SESSION: {}".format(
                                os.path.join(bpod_export_dir, bpod_export_file)
                            )
                        )
