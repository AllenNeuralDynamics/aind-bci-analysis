...
def find_conditioned_neuron_idx(
    session_bpod_file, session_ops_file, fov_stats_file, plot=False
):
    """
    ...
    plot : bool, optional
        If set to true, mean image is shown along with suite2p ROIs, the found
          conditioned ROI is yellow.
        Red dots are scanimage roi integration centers, one should overlap with
         the conditioned ROI.
    Returns
    -------
    cond_s2p_idx : list of int
        one number for each trial, which was the conditioned neuron -  should
          be the same for the whole session
    ...
    Example
    -------
    session_bpod_file = '/mnt/Data/Behavior/BCI_exported/KayvonScope/BCI_35/
                          072122-bpod_zaber.npy'
    session_ops_file = '/home/rozmar/Network/GoogleServices/BCI_data/Data/
                         Calcium_imaging/suite2p/Bergamo-2P-Photostim/BCI_35/
                         FOV_05/072122/ops.npy'
    fov_stats_file = '/home/rozmar/Network/GoogleServices/BCI_data/Data/
                       Calcium_imaging/suite2p/Bergamo-2P-Photostim/BCI_35/
                       FOV_05/stat.npy'
    cond_s2p_idx,closed_loop_trial,scanimage_filenames = (
      find_conditioned_neuron_idx(session_bpod_file,session_ops_file,
                                  fov_stats_file, plot = True))
    """

    behavior_dict = np.load(session_bpod_file, allow_pickle=True).tolist()
    ops = np.load(session_ops_file, allow_pickle=True).tolist()
    meanimg_dict = np.load(
        os.path.join(os.path.dirname(session_ops_file), "mean_image.npy"),
        allow_pickle=True,
    ).tolist()
    ...
    closed_loop_trial = []
    scanimage_filenames = []
    for i, (filename, tiff_header) in enumerate(
        zip(
            behavior_dict["scanimage_file_names"],
            behavior_dict["scanimage_tiff_headers"],
        )
    ):  # find names of conditioned neurons
        ...
        if len(behavior_dict["scanimage_roi_outputChannelsRoiNames"][i]) > 0:
            ...
            for roi_fcn_i, roi_fnc_name in enumerate(
                behavior_dict["scanimage_roi_outputChannelsNames"][i]
            ):
            ...
            try:
                conditioned_neuron_name = (
                    behavior_dict["scanimage_roi_outputChannelsRoiNames"][i][
                        bpod_analog_idx
                    ]
                )[0]
            # TODO: Don't use bare except.
            except:  # noqa: E722
                conditioned_neuron_name = behavior_dict[
                    "scanimage_roi_outputChannelsRoiNames"
                ][i][bpod_analog_idx]
            if len(conditioned_neuron_name) == 0:
                conditioned_neuron_name = ""
            rois = metadata["metadata"]["json"]["RoiGroups"][
                "integrationRoiGroup"
            ]["rois"]
            ...
            try:
                roi_idx = (
                    np.where(
                        np.asarray(roinames_list) == conditioned_neuron_name
                    )[0][0]
                    + 1
                )
            # TODO: Don't use bare except.
                ...
                try:
                    print(
                        "ROI names in scanimage header does not match up: "
                        "{}".format(conditioned_neuron_name)
                    )
                    conditioned_neuron_name = " ".join(
                        conditioned_neuron_name.split(",")
                    )
                    roi_idx = (
                        np.where(
                            np.asarray(roinames_list)
                            == conditioned_neuron_name
                        )[0][0]
                        + 1
                    )
                # TODO: Don't use bare except. Which exception is being caught?
                except:  # noqa: E722
        ...
        if (
            roi_idx is not None
            and metadata["metadata"]["hIntegrationRoiManager"]["enable"]
    ...
    fovdeg = list()
    for s in (
        metadata["metadata"]["hRoiManager"]["imagingFovDeg"]
        .strip("[]")
        .split(" ")
    ):
        fovdeg.extend(s.split(";"))
    fovdeg = np.asarray(fovdeg, float)
    fovdeg = [np.min(fovdeg), np.max(fovdeg)]
    rois = metadata["metadata"]["json"]["RoiGroups"]["integrationRoiGroup"][
        "rois"
    ]
    if type(rois) == dict:
        rois = [rois]
    ...
    yup, xup = upsample_block_shifts(
        ...
        ops["xblock"],
        ops["yblock"],
        np.median(ops["yoff1_list"][:5000, :], 0)[np.newaxis, :],
        np.median(ops["xoff1_list"][:5000, :], 0)[np.newaxis, :],
    )
        ...
        try:
            ...
            if "rotation_deg" in meanimg_dict.keys():
                ...
                angle = -1 * np.mean(
                    np.asarray(
                        [
                            np.arccos(meanimg_dict["rotation_matrix"][0, 0]),
                            np.arcsin(meanimg_dict["rotation_matrix"][1, 0]),
                            -1
                            * np.arcsin(meanimg_dict["rotation_matrix"][0, 1]),
                            np.arccos(meanimg_dict["rotation_matrix"][1, 1]),
                        ]
                    )
                )
                # print('offset: {} pixels, rotation {}
                # degrees'.format([x_offset,y_offset], np.degrees(angle)))
                ox = oy = 0
                qx = (
                    ox
                ...
                qx = px
                qy = py
            coordinates_now = (np.asarray([qx, qy]) - fovdeg[0]) / np.diff(
                fovdeg
            )
            # centerXY_list.append((roi['scanfields']['centerXY']-
            # fovdeg[0])/np.diff(fovdeg))
        # TODO: Don't use bare except. Which exception is being caught?
        except:  # noqa: E722
            ...
            if "rotation_deg" in meanimg_dict.keys():
                ...
                print(
                    "rotating ROIs with {} degrees".format(np.degrees(angle))
                )
                ox = oy = 0
                qx = (
                    ox
    ...
    for roi_idx_now in roi_indices:
        ...
        for cell_stat in stat:
            ...
            dist = np.sqrt(
                (centerXY_list[roi_idx_now - 1][0] - cell_stat["med"][1]) ** 2
                + (centerXY_list[roi_idx_now - 1][1] - cell_stat["med"][0])
                ** 2
            )  # - cell_stat['radius']
    ...
    if plot:
        ...
        ax_rois = fig_meanimage.add_subplot(
            2, 1, 2, sharex=ax_meanimage, sharey=ax_meanimage
        )
        ...
        for i, roi_stat in enumerate(stat):
            if (
                i
                == np.unique(
                    np.asarray(cond_s2p_idx)[
                        np.asarray(cond_s2p_idx) is not None
                    ]
                )[0]
            ):
            ...
            # break
        ax_rois.plot(
            np.asarray(centerXY_list)[:, 0],
            np.asarray(centerXY_list)[:, 1],
            "ro",
        )
