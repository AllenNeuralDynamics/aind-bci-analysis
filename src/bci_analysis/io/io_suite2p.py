...
def trial_times_to_session_indices(
    all_trial_frame_nums,
    all_trial_names,
    ...
    -------
    import BCI_analysis
    file_path = ('/home/rozmar/Network/GoogleServices/BCI_data/Data/'
             'Calcium_imaging/sessionwise_tba/BCI_26/BCI_26-060722-FOV_06.npy')
    ...
    event_indices = trial_times_to_session_indices(all_trial_frame_nums,
                                                   ...
                                                   frame_rate)
    """
    all_trial_start_frames = np.asarray(
        np.concatenate([[0], np.cumsum(all_trial_frame_nums)])
    )[:-1]

    event_indices = []
    for trial_i, (trial_name, event_times) in enumerate(
        zip(trial_names_with_event, trial_event_times)
    ):
        trial_start_idx = all_trial_start_frames[
            np.where(trial_name == np.asarray(all_trial_names))[0][0]
        ]
...
def align_trace_to_event(F, event_indices, frames_before, frames_after):
    """
    ...
    for i, center_idx in enumerate(event_indices):
        ...
        F_aligned[
            missing_frames_at_beginning : missing_frames_at_beginning
            + end_frame
            - start_frame,
            :,
            i,
        ] = F[:, start_frame:end_frame].T
    return F_aligned


def sessionwise_to_trialwise(
    F,
    all_si_filenames,
    ...
    """
    this function does not care if there is a movie for the trial or not,
      handle this somewhere else
    """
    align_keys = ["go_cue", "reward", "trial_start"]
    if align_on not in align_keys:
        print(
            "select reference (align_on) from this list: {}".format(align_keys)
        )
        output = None
        ...
        #     print("Aligning with go_cue or trial_start requires frame"
        #           " numbers to take")
        #     return
        ...
        if max_frames == "all":
            print(
                "Since max_frames is all, this function will return a list of "
                "F trialwise as all trials have different lengths"
            )
            F_trialwise_closed_loop = []
            counter = 0
            for i, filename in enumerate(all_si_filenames):
                if filename not in closed_loop_filenames:
                    continue
            ...
            F_trialwise_closed_loop = (
                np.ones((max_frames, F.shape[0], len(closed_loop_filenames)))
                * np.nan
            )
            counter = 0
            for i, filename in enumerate(all_si_filenames):
                ...
                )  # taking 40 time points before trial starts
                if start_frame < 0:  # taking care of edge at the beginning
                    missing_frames_at_beginning = np.abs(start_frame)
                    start_frame = 0
                else:
...
def suite2p_to_npy(
    ...
    """
    This script saves the relevant data into .npy files that can be analysed
    later
    ----------
    suite2p_path : str
    ...
    max_frames: int
        Number of frames from the current trial that will appear in the
        trialwise matrices
    frames_prev_trial: int
        Number of frames from previous trial that will appear in the
        trialwise matrices
    ...
    behavior_data_path = ("bucket/Data/Behavior/BCI_exported/"
                          "Bergamo-2P-Photostim/")
    save_path = "/home/jupyter/bucket/Data/Calcium_imaging/sessionwise_tba"
    suite2p_to_npy(suite2p_path, raw_data_path, behavior_data_path, save_path,
                   overwrite=True, mice_name = mice_name)
    """
    ...
    for mouse in mice_name:
        ...
        if fov_list is None:
            ...
            fov_list = [
                k for k in os.listdir(suite2p_data) if k[-2:].isdigit()
            ]  # For BCI_29 there exists folders other than FOV_0x

        for fov in fov_list:
            fov_path = os.path.join(suite2p_data, fov)
            try:
                mean_image = np.load(os.path.join(fov_path, "mean_image.npy"))
                # TODO: the mean image should come from the session and not
                #       from the FOV
            except OSError:
                print(f"Files Not present for this {fov}, skipping")
                continue
            max_image = np.load(os.path.join(fov_path, "max_image.npy"))
            # TODO: the max image should come from the session and not from
            #       the FOV
            stat = np.load(
                os.path.join(fov_path, "stat.npy"), allow_pickle=True
            ).tolist()
            ...
            for session_date in session_list:
                ...
                session_save_path = os.path.join(
                    mice_save_path, f"{mouse}-{session_date}-{fov}.npy"
                )
                if os.path.exists(session_save_path) and overwrite is False:
                    print(
                        f"Session already exists at {session_save_path}, "
                    ...
                    tsta = (
                        np.arange(
                            -frames_prev_trial, frames_this_trial, 1
                        ).astype(float)
                        / fs
                    )

                    F = np.load(
                        os.path.join(session_path, "F.npy"), allow_pickle=True
                    )
                    F0 = np.load(
                        os.path.join(session_path, "F0.npy"), allow_pickle=True
                    )
                    photon_counts_dict = np.load(
                        os.path.join(session_path, "photon_counts.npy"),
                        allow_pickle=True,
                    ).tolist()

                    f0_scalar = np.percentile(
                        F0[:, : int(F0.shape[1] / 2)], 10, axis=1
                    )

                    if (
                        adjust_channel_offsets
                    ):  # this is optional, might not be important
                        channel_offset_dict = np.load(
                            os.path.join(session_path, "channel_offset.npy"),
                            allow_pickle=True,
                        ).tolist()
                    ...
                    with open(
                        os.path.join(session_path, "filelist.json")
                    ) as json_file:
                        filelist = json.load(json_file)

                    all_si_filenames = filelist["file_name_list"]
                    behavior_fname = os.path.join(
                        behavior_data_path, f"{session_date}-bpod_zaber.npy"
                    )
                    (
                        cn_idx,
                        _closed_loop_trial,
                        _scanimage_filenames,
                    ) = find_conditioned_neuron_idx(
                        behavior_fname,
                        os.path.join(session_path, "ops.npy"),
                    ...
                    try:
                        clt = np.concatenate(
                            np.asarray(_scanimage_filenames)[
                                _closed_loop_trial
                            ]
                        )
                    ...
                    closed_loop_filenames = [
                        k
                        for k in filelist["file_name_list"]
                        if k.lower().startswith("neuron") or k in clt
                    ]
                    # TODO: we should pull out this information from the
                    #       scanimage tiff header
                    # closed_loop_filenames = (
                    #   [k[0]
                    #     for k
                    #     in
                    #     np.asarray(_scanimage_filenames)[_closed_loop_trial]
                    #     if k[0].lower().startswith("neuron")])

                    frame_num = np.asarray(filelist["frame_num_list"])
                    filename_start_frame = np.asarray(
                        [0] + np.cumsum(frame_num).tolist()
                    )

                    F_trialwise_all = (
                        np.ones(
                            (max_frames, F.shape[0], len(all_si_filenames))
                        )
                        * np.nan
                    )
                    F_trialwise_closed_loop = (
                    ...
                    for i, filename in enumerate(all_si_filenames):
                        ...
                        start_frame = (
                            start_frame - frames_prev_trial
                        )  # taking 40 time points before trial starts
                        if (
                            start_frame < 0
                        ):  # taking care of edge at the beginning
                            missing_frames_at_beginning = np.abs(start_frame)
                            start_frame = 0
                        else:
                            missing_frames_at_beginning = 0
                    ...
                    roi_centers = [
                        (stat[i]["xpix"].mean(), stat[i]["ypix"].mean())
                    ...
                    dist = [
                        np.sqrt(
                            roi_centers_cn[i][0] ** 2
                            + roi_centers_cn[i][1] ** 2
                        )
                        for i in range(len(roi_centers_cn))
                    ]

                    if not os.path.isfile(behavior_fname):
                        print(
                            "No corresponding behavior data found for "
                            "{}".format(session_date)
                        )
                        break
                    else:
                        bpod_zaber_data = np.load(
                            behavior_fname, allow_pickle=True
                        ).tolist()
                        files_with_movies = []
                        ...
                        trial_times = [
                            (trial_et[i] - trial_st[i]).total_seconds()
                            for i in range(len(trial_st))
                        ]
                        ...
                        threshold_crossing_times = bpod_zaber_data[
                    ...
                    dict_all = {
                        ...
                        "zaber_move_forward": bpod_zaber_data[
                            "zaber_move_forward"
                        ][files_with_movies],
                        "sampling_rate": fs,
                        ...
                        "scanimage_filenames": bpod_zaber_data[
