...
def calculate_step_time(s, v, a):
    """
    This script calculates zaber step times from acceleration, max speed and
    distance to travel.
...
def load_dir_stucture(
    ...
    for projectname in projectdir.iterdir():
        if projectname.is_dir() and (
            not projectnames_needed or projectname.name in projectnames_needed
        ):
            ...
            for experimentname in (projectname / "experiments").iterdir():
                if experimentname.is_dir() and (
                    not experimentnames_needed
                    or experimentname.name in experimentnames_needed
                ):
                    dirstructure[projectname.name][
                        experimentname.name
                    ] = dict()
                    experimentnames.append(experimentname.name)

                    for setupname in (experimentname / "setups").iterdir():
                        if setupname.is_dir() and (
                            not setupnames_needed
                            or setupname.name in setupnames_needed
                        ):
                            ...
                            for sessionname in (
                                ...
                                if sessionname.is_dir():
                                    ...
                                    dirstructure[projectname.name][
                                        experimentname.name
                                    ][setupname.name].append(sessionname.name)
    out_dict = {
...
def pybpod_csv_to_dataframe(csvfilename, subject_needed=""):
    ...
    df = df[df["TYPE"] != exception_str]  # delete empty rows
    ...
    idxdiff = np.diff(idx)
    Trialnum = np.array([])
    for i, idxnumnow in enumerate(
        idxdiff
    ):  # zip(np.arange(0:len(idxdiff)),idxdiff):#
    ...
    if len(indexes) > 0:
        ...
        for trialnumber_real, trialnum in zip(trialnumbers_real, trialnumbers):
            # df['Trial_number'][df['Trial_number_in_session'] == trialnum]
            # = int(blocknumber)
            try:
                df.loc[
                    df["Trial_number_in_session"] == trialnum, "Trial_number"
                ] = (int(trialnumber_real) - 1)
                # Note that in the pybpod protocol the trial number comes
                # BEFORE the go cue, so the numbering is off by one
            # TODO: Don't use bare except.
            except:  # noqa: E722
                df.loc[
                    df["Trial_number_in_session"] == trialnum, "Block_number"
                ] = np.nan
    ...
    for variableidx in variableidxs:
        ...
        for varname in d["variables"].keys():
            if isinstance(d["variables"][varname], (list, tuple)):
                ...
                df.loc[range(variableidx, len(df)), "var:" + varname] = d[
                    "variables"
                ][varname]
...
def pybpod_dataframe_to_dict(data):
    ...
    Zaber_moves_channel = [
        "Wire1High",
        "Wire1Low",
    ]  # TODO this is hard coded, should be in the variables
    ...
    for trial_num, (trial_start_idx, trial_end_idx) in enumerate(
        zip(trial_start_idxs, trial_end_idxs)
    ):
        df_trial = data[trial_start_idx:trial_end_idx]
        try:
            df_past_trial = data[
                trial_end_idx : trial_start_idxs[trial_num + 1]
            ]
        # TODO: Don't use bare except.
        except:  # noqa: E722
        ...
        for past_trial_line in df_past_trial.iterrows():
            ...
            if "Movie names for trial:" in str(past_trial_line["MSG"]):
                behavior_movie_names = (
                    past_trial_line["MSG"][23:].strip("[]").split(",")
                )
        ...
        go_cue_time = df_trial.loc[
            (df_trial["MSG"] == "GoCue") & (df_trial["TYPE"] == "TRANSITION"),
            "BPOD-INITIAL-TIME",
        ].values  # [0]#.index.to_numpy()[0]
        threshold_crossing_time = df_trial.loc[
            (df_trial["MSG"] == "ResponseInRewardZone")
            & (df_trial["TYPE"] == "TRANSITION"),
            "BPOD-INITIAL-TIME",
        ].values  # [0]#.index.to_numpy()[0]
        if len(go_cue_time) == 0:
            continue  # no go cue no trial

        lick_left_times = df_trial.loc[
            data["var:WaterPort_L_ch_in"] == data["+INFO"], "BPOD-INITIAL-TIME"
        ].values
        lick_right_times = df_trial.loc[
            data["var:WaterPort_R_ch_in"] == data["+INFO"], "BPOD-INITIAL-TIME"
        ].values
        lick_left_times_end = df_trial.loc[
            data["var:WaterPort_L_ch_out"] == data["+INFO"],
            "BPOD-INITIAL-TIME",
        ].values
        lick_right_times_end = df_trial.loc[
            data["var:WaterPort_R_ch_out"] == data["+INFO"],
            "BPOD-INITIAL-TIME",
        ].values
        reward_left_times = df_trial.loc[
            (data["MSG"] == "Reward_L") & (data["TYPE"] == "TRANSITION"),
            "BPOD-INITIAL-TIME",
        ].values
        reward_right_times = df_trial.loc[
            (data["MSG"] == "Reward_R") & (data["TYPE"] == "TRANSITION"),
            "BPOD-INITIAL-TIME",
        ].values
        autowater_left_times = df_trial.loc[
            (data["MSG"] == "Auto_Water_L") & (data["TYPE"] == "TRANSITION"),
            "BPOD-INITIAL-TIME",
        ].values
        autowater_right_times = df_trial.loc[
            (data["MSG"] == "Auto_Water_R") & (data["TYPE"] == "TRANSITION"),
            "BPOD-INITIAL-TIME",
        ].values
        zaber_motor_movement_times = df_trial.loc[
            (Zaber_moves_channel[0] == data["+INFO"])
            | (Zaber_moves_channel[1] == data["+INFO"]),
            "BPOD-INITIAL-TIME",
        ].values
        trial_number = df_trial.loc[
            (df_trial["MSG"] == "GoCue") & (df_trial["TYPE"] == "TRANSITION"),
            "Trial_number",
        ].values[0]
        ...
        for key_now in data.keys():
            if "var:" in key_now:
                data_dict[key_now.replace(":", "_")].append(
                    df_trial.loc[
                        (df_trial["MSG"] == "GoCue")
                        & (df_trial["TYPE"] == "TRANSITION"),
                        key_now,
...
def add_zaber_info_to_pybpod_dict(
    behavior_dict, subject_name, setup_name, zaber_folder_root
):
    """
    extracts zaber and BCI settings (generated by the Zaber gui) from zaber
    json files and aligns it to trials.
    ...
    try:
        zaberdir = os.path.join(
            zaber_folder_root, setup_dirname, "subjects", subject_name
        )
    ...
    for trial_start_time in behavior_dict["trial_start_times"]:
        ...
        if zaber_file_idx != zaber_file_idx_prev:
            with open(
                os.path.join(zaberdir, zaberfiles[zaber_file_idx]), "r"
            ) as read_file:
    ...
    for v, a, s in zip(
        zaber_vars_dict["speed"],
        zaber_vars_dict["acceleration"],
        zaber_vars_dict["trigger_step_size"],
    ):
