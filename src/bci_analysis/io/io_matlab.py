...
def read_multisession_mat(full_file_path):
    ...
    data_dict : dict
        ...
        dff_trialwise_closed_loop - float
            [sessions x time points x neurons x trials]
    ...
    data_dict = {
        ...
        "session_dates": [
            datetime.strptime(str(int(i)).zfill(6), "%M%d%Y").date()
            for i in data["sessionDate"]
        ],
        ...
        "conditioned_neuron_idx": (
            np.asarray(data["cn"], int).flatten() - 1
        ).tolist(),  # matlab to python indexing
        "dff_sessionwise_all_epochs": data["dff_sessionwise_all_epochs"],
        "dff_sessionwise_closed_loop": data["dff_sessionwise_closed_loop"],
        "dff_trialwise_closed_loop": data[
            "dff_trialwise_closed_loop"
        ],  # this one is missing from many files, commented out for now
