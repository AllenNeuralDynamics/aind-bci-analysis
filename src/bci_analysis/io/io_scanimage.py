...
def extract_scanimage_metadata(file):
    ...
    description_first_image_dict = dict(
        item.split(" = ")
        for item in description_first_image.rstrip(r"\n ")
        .rstrip("\n")
        .split("\n")
    )
    ...
    metadata_dict = dict(
        item.split("=") for item in metadata_str.split("\n") if "SI." in item
    )
    metadata = {
        k.strip().replace("SI.", ""): v.strip()
    ...
    for k in list(metadata.keys()):
        if "." in k:
            ...
            if k.count(".") == 1:
                ...
                print(
                    "skipped metadata key "
                    + k
                    + " to minimize recursion in dict"
                )
            metadata.pop(k)
    ...
    for r in range(num_rois):
        ...
        # print('{} {} {}'.format(roi[r]['w_px'], roi[r]['h_px'],
        #       roi[r]['size']))
    ...
    if data["mesoscan"]:
        ...
        for i in range(num_rois):
            ...
            data["lines"] = list(
                range(
                    irow[0, i].astype("int32"), irow[1, i].astype("int32") - 1
                )
    ...
    movie_start_time = datetime.datetime.strptime(
        description_first_image_dict["epoch"].rstrip("]").lstrip("["),
        "%Y %m %d %H %M %S.%f",
    )
    out = {
        "metadata": metadata,
