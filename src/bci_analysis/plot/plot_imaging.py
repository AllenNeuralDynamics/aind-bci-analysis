...
def plot_trial_averaged_trace_2sessions(
    data_dict,
    day_ind,
    start_time=-2.5,
    end_time=10,
    ...
    avg_traces_yesterday = np.nanmean(
        data_dict["F_trialwise"][day_ind - 1][idx_yesterday, :, :], axis=2
    )
    avg_traces_today = np.nanmean(
        data_dict["F_trialwise"][day_ind][idx_today, :, :], axis=2
    )

    baseline_idx_today = (time_today > baseline_start_time) & (
        time_today < baseline_end_time
    )
    baseline_idx_yesterday = (time_yesterday > baseline_start_time) & (
        time_yesterday < baseline_end_time
    )

    for i in range(avg_traces_today.shape[1]):
        avg_traces_today[:, i] = avg_traces_today[:, i] - np.nanmean(
            avg_traces_today[baseline_idx_today, i], 0
        )
        avg_traces_yesterday[:, i] = avg_traces_yesterday[:, i] - np.nanmean(
            avg_traces_yesterday[baseline_idx_yesterday, i], 0
        )

    plt.plot(
        time_today, avg_traces_today[:, data_dict["cni"][day_ind]], color="r"
    )
    plt.plot(
        time_yesterday,
        avg_traces_yesterday[:, data_dict["cni"][day_ind]],
        color="k",
    )
